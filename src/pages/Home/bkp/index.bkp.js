...
export default class Home extends Component {
  ...
  handlePressShow(mesa, i) {
    const { show, mesasAtiva } = this.state;
    const shows = show;
    shows[i] = !show[i];
    this.setState({
      show: shows,
    ...
    ativa
      ...
      : this.setState({ mesasAtiva: [...mesasAtiva, mesa] });
    console.log(show);
    console.log(mesasAtiva);
  }

  handlePressHidden() {
    const { mesaAtual, mesas, show } = this.state;
    console.log(`Proccess Hidden => atual: ${mesaAtual}`);
    const shows = show;
    mesas.map((mesa, i) => {
      if (mesa.id === mesaAtual) {
        shows[i] = !show[i];
        this.setState({ show: shows, mesaAtual: 0 });
      }
    });
  }
          ...
          >
            ...
            >
                ...
                <ScrollView horizontal>
                  {mesasAtiva.map((mesaAtiva, index) => (
                    <TouchableOpacity
                      ...
                      onPress={() => this.handlePressShow(mesaAtiva, index)}
                    >
              ...
              <TouchableOpacity
                ...
                onPress={() => this.handlePressHidden()}
              >
          ...
          <View style={{ flex: 6, backgroundColor: 'green' }}>
            ...
            >
              {mesas.map((mesa, index) => (
                <Content
                  onPress={() => this.handlePressShow(mesa.id, index)}
                  activeOpacity={0.8}
                  key={mesa.id}
                >
                  <Image
