...
/** @define {string} */
const AUTOBAHNJS_VERSION = '?.?.?';

/** @define {boolean} */
const AUTOBAHNJS_DEBUG = true;
...
ab.PrefixMap.prototype.shrink = function (uri, pass) {
  ...
  if (i == -1) {
    for (var i = uri.length; i > 0; i -= 1) {
      const u = uri.substring(0, i);
      const p = self._rindex[u];
      if (p) {
    ...
    if (o[1] in self._calls) {
      if (o[0] === ab._MESSAGE_TYPEID_CALL_RESULT) {
        ...
        if (ab._debugrpc && dr._ab_callobj !== undefined) {
          ...
          for (let i = 3; i < dr._ab_callobj.length; i += 1) {
            const arg = dr._ab_callobj[i];
            if (arg !== undefined) {
              console.log(arg);
            } else {
              break;
        ...
        if (ab._debugrpc && de._ab_callobj !== undefined) {
          ...
          console.group('Arguments');
          for (let j = 3; j < de._ab_callobj.length; j += 1) {
            const arg2 = de._ab_callobj[j];
            if (arg2 !== undefined) {
              console.log(arg2);
            } else {
              break;
      ...
      const subid = self._prefixes.resolve(o[1], true);
  ...
  self._websocket.onclose = function (e) {
    if (ab._debugws) {
      if (self._websocket_connected) {
        console.log(
          `Autobahn connection to ${self._wsuri} lost (code ${e.code}, reason '${e.reason}', wasClean ${e.wasClean}).`
        );
      } else {
        console.log(
          `Autobahn could not connect to ${self._wsuri} (code ${e.code}, reason '${e.reason}', wasClean ${e.wasClean}).`
        );
      }
    }
...
ab.Session.prototype.prefix = function (prefix, uri) {
  ...
  const msg = [ab._MESSAGE_TYPEID_PREFIX, prefix, uri];
  self._send(msg);
};
...
ab.Session.prototype.call = function () {
  ...
  const procuri = self._prefixes.shrink(arguments[0], true);
  const obj = [ab._MESSAGE_TYPEID_CALL, callid, procuri];
...
ab.Session.prototype.subscribe = function (topicuri, callback) {
  ...
  //
  const rtopicuri = self._prefixes.resolve(topicuri, true);
  if (!(rtopicuri in self._subscriptions)) {
    ...
    const msg = [ab._MESSAGE_TYPEID_SUBSCRIBE, topicuri];
    self._send(msg);
  ...
  //
  const i = self._subscriptions[rtopicuri].indexOf(callback);
  if (i === -1) {
...
ab.Session.prototype.unsubscribe = function (topicuri, callback) {
  ...
  if (!(rtopicuri in self._subscriptions)) {
    ...
    if (callback !== undefined) {
      const idx = self._subscriptions[rtopicuri].indexOf(callback);
      if (idx !== -1) {
        removed = callback;
    ...
    if (self._subscriptions[rtopicuri].length === 0) {
      ...
      const msg = [ab._MESSAGE_TYPEID_UNSUBSCRIBE, topicuri];
      self._send(msg);
    }
  }
};
